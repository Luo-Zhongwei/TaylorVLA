...
# 1) 将项目根加入搜索路径（如有必要）
sys.path.append(os.path.expanduser("/root/autodl-tmp/RoboticsDiffusionTransformer"))

# 2) 启动 debugpy，监听 5678 端口，并等待 VS Code 附加
debugpy.listen(5678)

from colorama import init
from termcolor import colored
init()  # 在 Windows 上初始化支持
print(colored(f">> Debugpy is listening on port 5678, waiting for VS Code to attach ...", "red"))


#print(">> Debugpy is listening on port 5678, waiting for VS Code to attach ...")  # 可选提示
...
# def main():
#     print("begain to enter main")
#     # set cuda 
#     args = parse_args()
#     # set random seeds
#     seed = args.random_seed
    
#     print("begain to hit break point")
#     #debugpy.breakpoint()
...
#     from termcolor import colored
#     init()  # 在 Windows 上初始化支持
#     filename = f'text_embed_{env_id}.pt'
#     if os.path.exists(f'text_embed_{env_id}.pt'):
#         full_path = os.path.abspath(filename)
#         print(colored(f"load text embed", "red"))
#         text_embed = torch.load(f'text_embed_{env_id}.pt')
#     else:
#         print(colored(f"computed text embed", "red"))
...
# —— 3. 模拟命令行参数 —— #
# 让 runpy 执行模块时，sys.argv 如同 `python -m eval_sim.eval_rdt_maniskill [args...]`
sys.argv = [
    "eval_sim.eval_rdt_maniskill",               # 模块名占位
    "--env-id", "PickCube-v1",                   # 示例参数
    "--pretrained_path", "/root/autodl-tmp/ckpt/rdt/maniskill-model/rdt/mp_rank_00_model_states.pt"
]

# —— 4. 运行目标模块的 __main__ 逻辑 —— #
# run_name="__main__" 等同于 -m 的执行方式
# alter_sys=True 会将 sys.argv[0] 替换为实际模块文件路径:contentReference[oaicite:2]{index=2}
runpy.run_module(
    "eval_sim.eval_rdt_maniskill",
    run_name="__main__",
    alter_sys=True
)