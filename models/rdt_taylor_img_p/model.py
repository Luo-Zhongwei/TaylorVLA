...
from models.rdt.blocks import  RDTBlock
# from models.rdt_taylor.blocks import (FinalLayer, RDTBlock_taylor, TimestepEmbedder,
#                                get_1d_sincos_pos_embed_from_grid,
#                                get_multimodal_cond_pos_embed)

from models.rdt_taylor_img_p.blocks import (FinalLayer, RDTBlock_taylor_img_p, TimestepEmbedder,
                               get_1d_sincos_pos_embed_from_grid,
                               get_multimodal_cond_pos_embed)



from  models.taylor.cache_functions import cal_type
...
class RDT(nn.Module):
    ...
    def forward(self, x, freq, t, lang_c, img_c, lang_mask=None, img_mask=None):
        ...
        # print(f"[yellow]full out：{r['ms']:.3f}ms[/yellow]")
        x = self.final_layer(x)
...
class RDT_taylor_img_p(nn.Module):
    """
    Class for Robotics Diffusion Transformers.
    """
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            RDTBlock_taylor_img_p(hidden_size, num_heads,time_records=self.time_records) for _ in range(depth)
        ])
    ...
    def forward(self,current ,cache_dic ,
                ...
                add_img_pos: bool = True,
                img_keep_idx: torch.LongTensor | None = None):
    ...
    #             x, freq, t, lang_c, img_c, lang_mask=None, img_mask=None): 
        ...
        if add_img_pos:
            ...
            # 2) 图像 pos embedding：只需一次 gather
            if img_keep_idx is not None:
                B, K = img_keep_idx.shape
                pos_full = self.img_cond_pos_embed[:, :self.img_cond_len, :]
                batch_idx = torch.arange(B, device=img_c.device)[:, None]
                pos_pruned = pos_full.expand(B, -1, -1)[batch_idx, img_keep_idx, :]  # (B, K, D)
                img_c = img_c + pos_pruned
            else:
                img_c = img_c + self.img_cond_pos_embed[:, :img_c.shape[1], :]

        # Forward pass
        conds = [lang_c, img_c]
        masks = [lang_mask, img_mask]                                # (N, D)
        ...
        for i, block in enumerate(self.blocks):
            current['layer'] = i 
            #print("in RDT model blocks: ",i)
            c, mask = conds[i%2], masks[i%2]
            x = block(x, c, mask,cache_dic=cache_dic,
                     current=current)                       # (B, T+1, D)
        # Inject the language condition at the final layer
        # with cuda_timer("full atten") as r:
        #     x = self.final_layer(x)  
        # print(f"[yellow]full out：{r['ms']:.3f}ms[/yellow]")                         # (B, T+1, out_channels)
        # self.time_records["flayer"].append(r['ms'])
        x = self.final_layer(x)
