...
#global_taylor_time_flag
gttf=False
#cuda_record_layer_flag
crlf=False
import contextlib
...
#                                 RDT Block                                     #
#################################################################################
class RDTBlock_taylor_img_p(nn.Module):
    """
    ...
    def forward(self, x, c, mask=None, *, cache_dic, current):
        ...
        if current['type'] == 'full':
            if current['layer']==0 and gttf:
                a=1
                #console.print(f"full: ", "blue"+ f"step:{current['step']:.3f}  layer:{current['layer']:.3f}", style="bold red") 
            ...
            if crlf:
                ...
                # --- 记录 full self-attention 耗时 ---
                global_profiler.record('full-sa', r['ms']) # <--- 在这里记录
            else:
                taylor_cache_init(cache_dic, current)
                sa_out = self.attn(x_norm, cache_dic, current) # ★ 把两个字典传进去
                derivative_approximation(cache_dic, current, sa_out)
            if gttf:
                console.print(f"full sa：{r['ms']:.3f}ms", style="bold red") 
            ...
            if current['layer']==0 and gttf:
                a=1
                #console.print(f"Taylor: ", "blue"+ f"step:{current['step']:.3f}  layer:{current['layer']:.3f}", style="bold red") 
                
            if crlf:
                with cuda_timer("full atten") as r:
                    sa_out = taylor_formula(cache_dic, current, order_limit=order)
                if gttf:
                    console.print(f"taylor sa：{r['ms']:.3f}ms", style="bold red") 
                self.time_records['taylor-sa'].append(r['ms'])
                global_profiler.record('taylor-sa', r['ms']) # <--- 在这里记录
        ...
        if current['type'] == 'full':
            if current['layer'] == 0 and gttf:
                a=1
                #console.print(f"full (cross): ", "blue" +f"step:{current['step']:.3f}  layer:{current['layer']:.3f}", style="bold red") 
            ...
            if crlf:   
                ...
                # --- 记录 full cross-attention 耗时 ---
                global_profiler.record('full-ca', r['ms']) # <--- 在这里记录
                if gttf:
                    console.print(f"full cross_a：{r['ms']:.3f}ms", style="bold red")
            else:
                taylor_cache_init(cache_dic, current)
                ca_out = self.cross_attn(q, c, mask,cache_dic, current)
                    #sa_out = self.attn(x_norm, cache_dic, current) # ★ 把两个字典传进去
            ...
            if current['layer'] == 0 and gttf:
                a=1
                #console.print(f" Taylor (cross): ", "blue" +f"step:{current['step']:.3f}  layer:{current['layer']:.3f}", style="bold red")
            if crlf:
                 
                with cuda_timer("taylor cross_attn") as r:
                    ca_out = taylor_formula(cache_dic, current,order_limit=order)
                if gttf:
                    console.print(f"taylor cross_a：{r['ms']:.3f}ms", style="bold red")
                    
                
                self.time_records['taylor-ca'].append(r['ms'])
                # --- 记录 taylor cross-attention 耗时 ---
                global_profiler.record('taylor-ca', r['ms']) # <--- 在这里记录
            else:
                ca_out = taylor_formula(cache_dic, current,order_limit=order)
        ...
        if current['type'] == 'full' :
            if crlf:
                ...
                if gttf:
                    console.print(f"full mlp：{r['ms']:.3f}ms", style="bold red")
                
                self.time_records['full-mlp'].append(r['ms'])
                # --- 记录 full FFN/MLP 耗时 ---
                global_profiler.record('full-mlp', r['ms']) # <--- 在这里记录
            else:
                ffn_out = self.ffn(x_norm, cache_dic, current)
        else:
            order = current.get('taylor_order', None)
            if crlf:
                ...
                if gttf:
                    console.print(f"taylor mlp：{r['ms']:.3f}ms", style="bold red")
                self.time_records['taylor-mlp'].append(r['ms'])
                # --- 记录 taylor FFN/MLP 耗时 ---
                global_profiler.record('taylor-mlp', r['ms']) # <--- 在这里记录
            else:
                ffn_out = taylor_formula(cache_dic, current, order_limit=order)
        x = origin + ffn_out
        return x
